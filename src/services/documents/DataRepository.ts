...
import { MongoClient, ServerApiVersion, UpdateOptions } from 'mongodb';
...
export class DataRepository implements IDataRepository {
  ...
  // private server: Server;
    ...
    async init(): Promise<void> {
      this.server = new MongoClient('mongodb://localhost:27017');
      try {
        await this.server.connect();
        await this.server.db(this.dbName).command({ ping: 1 });
        this.db = this.server.db(this.dbName);
    ...
    saveTableConfig(tableConfig: TableConfig): Promise<any> {    
      ...
      return this.db.collection('tableConfig').insertOne(tableConfig);
    }
  ...
  saveCurrencyConfig(config:CurrencyConfig): Promise<any> {
    ...
    return this.db.collection('currencyConfig').insertOne(config);
  };
  ...
  async saveUser(user: User): Promise<void> {    
    ...
    if(result.modifiedCount !== 1) {
      throw new Error(`expecting modified count of 1 instead it is ${result.modifiedCount}`)
  ...
  saveChat(chatMessage: ChatMessage): Promise<any> {
    ...
    return this.db.collection('chatMessages').insertOne(chatMessage);
  }
    ...
    return collection.find({ tableId: tableId.toString() }).toArray()
      .then((arr: any) => {
        if (arr.length === 0) {
          ...
          return collection.insertOne(tableBalance)
  ...
  saveReconcilliationView(view: IReconcilliationView): Promise<any> {
    ...
    return this.db.collection('reconcilliationViews').insertOne(view);
  }
  ...
  deleteUserReconcilliation(guid: string): Promise<any> {
    ...
    return this.db.collection('tournmanets').insertOne(tournmanet)
   };
  ...
  saveTableStates(states:TableState[]): Promise<any[]> { 
    ...
    return Promise.all(states.map(state=>{
      ...
      return this.db.collection('tableState').insertOne(state)
    }));
  }
   ...
   saveTournamentResult(results:TournamentResult[]) : Promise<any> {    
    return Promise.all(results.map(result=>{      
      ...
      return this.db.collection('tournamentResults').insertOne(result)
    }));    
   }
  ...
  saveAddress(info: AddressInfo): any {        
    ...
    return this.db.collection('addressInfo').insertOne(info);
  }
  ...
  saveChangeSeatHistory(history:ChangeSeatHistory): Promise<any> { 
    ...
    return this.db.collection('changeSeatHistory').insertOne(history);
  };
  ...
  saveTableProcessorMessage(message:DbTableProcessorMessage): Promise<any>  { 
    ...
    return this.db.collection('tableProcessorMessages').insertOne(message);
  };
